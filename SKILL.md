---
name: vision-cli
description: Multi-backend Image + Video + TTS CLI (vg command). Image: Volcengine Ark 火山方舟 字节 Seedream (seedream=doubao-seedream-4-5-251128, seedream-lite=doubao-seedream-5.0-lite, seedream-legacy=doubao-seedream-4-0-250828). Video: Volcengine Ark Seedance 2.0 (seedance=doubao-seedance-2-0-260128, seedance-fast=doubao-seedance-2-0-fast-260128). TTS: Volcengine TTS V3 火山引擎豆包语音 (tts gen). Use when user wants to generate images, videos, or speech, check generation history/stats, or manage API configuration. Triggers on "vg", "vision-cli", "生成图片", "出图", "seedream", "字节出图", "豆包出图", "火山方舟", "ark", "生成视频", "出视频", "seedance", "视频生成", "语音合成", "文字转语音", "TTS", "tts", "音色", "voice".
---

# vision-cli — Multi-backend Image, Video & TTS CLI

CLI tool `vg` wraps Volcengine Ark image generation backend. Pick the model via `--model <alias>`:
...
- 参考图数量上限：Seedream 14
- 4K：Seedream 全系原生支持
- `--negative-prompt`：Seedream 自动折叠进 prompt
...
| `-i, --input` | Reference image, repeatable (上限 14) |
| `--ar` | Aspect ratio: `1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9` |
| `--res` | Resolution: `1K, 2K, 4K` (Seedream 4K 原生) |
| `--repeat` | Generate N copies, 1-8 |
| `--model` | `seedream, seedream-lite, seedream-legacy` (default from config or seedream) |
| `--negative-prompt` | Negative prompt（Seedream 折叠进 prompt） |

**像素尺寸换算**：Seedream 把 `--ar` × `--res` 自动映射到像素 size（如 `--ar 16:9 --res 4K` → `4096x2304`），按模型能力封顶。
...
History records include a `backend` field (`volcengine_ark`) alongside the resolved `model` id.
...
vg config set default_model seedream-lite  # seedream | seedream-lite | seedream-legacy
